    ...
    if request.method == 'POST':
        if form.validate_on_submit(): #if fields filled properly
            ...
            if datetime_input:
                ...
                date = datetime_obj.date()  # Extract date part (YYYY-MM-DD)
                time = datetime_obj.time()  # Extract time part (HH:MM:SS)
            else:
                date = None
                time = None
...
def history():
    if request.method == 'POST':
        ...
        print(f'Student Id: {student_id}, Equipment ID: {equipment_id}')
        conn = get_db_connection()
        cur = conn.cursor()
